...
function Home() {
  ...
  return (
    <main>
      ...
      <p>Store: {profile.user.name}</p>
      <p>Normalized payload: {payload.user.name}</p>
      <p data-result={ok ? "pass" : "fail"}>
        {ok ? "PASS" : "FAIL"} — export normalization{" "}
        {ok ? "left the live store alone" : "mutated the live store"}.
      </p>
    </main>
  );
}
