...
function Profile() {
  ...
  const card = createMemo(() => {
    ...
    firstId ??= id;
    return { name: user(), firstId, finalId: id };
  });
  const ok = () => card().firstId === card().finalId;
  return (
    <p data-result={ok() ? "pass" : "fail"}>
      {card().name}: first id {card().firstId}, final id {card().finalId} — {ok() ? "PASS" : "FAIL"}
    </p>
  );
}
function Home() {
  return (
    <main>
      ...
      <p>An async retry must reuse the hydration-id slot allocated by its first attempt.</p>
