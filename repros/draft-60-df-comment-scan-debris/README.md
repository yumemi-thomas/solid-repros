...
integration as `draft-01`. There is no custom server, client entry, observer,
or analyzer: one route renders one async memo inside one `<Loading>` boundary,
using Start's normal `HydrationScript` + `Scripts` document shell.
...
the preview in Chrome. To exercise the fully-streamed-before-hydrate case,
block `*virtual:tanstack-start-dev-client-entry*` in DevTools Request Blocking
and reload. Inline stream scripts still run, but hydration does not start:

- Expected: `Download ready`
- Actual on beta.17: `Download ready42% complete`

Without blocking the external client entry, hydration may start early enough
to repair the slot; the malformed `$df` swap is still present in the streamed
response, but the permanent loaded-mode result is timing-dependent.
