...
const packageJson = slug => JSON.stringify({ name: slug, private: true, type: 'module', scripts: { dev: 'vite dev', start: 'vite dev', build: 'vite build' }, dependencies: { '@solidjs/web': '2.0.0-beta.17', 'solid-js': '2.0.0-beta.17' }, devDependencies: { vite: '^7.0.0', 'vite-plugin-solid': '3.0.0-next.7', typescript: '^5.8.3' } }, null, 2)
const vite = `import { defineConfig } from 'vite'\nimport solid from 'vite-plugin-solid'\nexport default defineConfig({ plugins: [solid()] })\n`
const tsconfig = JSON.stringify({ compilerOptions: { target: 'ESNext', module: 'ESNext', moduleResolution: 'Bundler', jsx: 'preserve', jsxImportSource: '@solidjs/web', strict: true, skipLibCheck: true, noEmit: true, isolatedModules: true, types: ['vite/client'], lib: ['DOM', 'DOM.Iterable', 'ESNext'] }, include: ['src/**/*.ts', 'src/**/*.tsx'] }, null, 2)

function section(markdown) { const a = markdown.indexOf('### Your Example Website or App'); const b = markdown.indexOf('### Steps to Reproduce', a); return markdown.slice(a, b) }
function blocks(markdown) { return [...section(markdown).matchAll(/```tsx\n([\s\S]*?)```/g)].map(match => match[1].trim()) }
